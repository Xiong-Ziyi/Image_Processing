...
using FFTW, Colors, ImageShow, TestImages, Tullio, Zygote, Noise, IndexFunArrays, FourierTools, PlutoUI, DeconvOptim, PlutoTest, Optim, Statistics, Plots
...
function LR(measured, psf, N)
	# TODO
	function conv(a, b)
		return real.(ift(ft(a) .* ft(b)))
	end
	
	img = copy(measured)
	
	for i in 1:N
		ratio = measured ./ conv(img, psf)
		img .*= conv(ratio, psf)
	end
...
# ╔═╡ 40099ec2-8fff-4c49-b296-083d1ded97ee
...
# ╔═╡ 481bccb6-ae79-407e-a476-e887779c8fde
gray_show(measured)
...
function TV(img::AbstractArray{T, N}, ϵ=T(1e-6)) where {T, N}
	...
	# TODO
	@tullio sum = sqrt(ϵ + abs2(img[i, j] - img[i+1, j]) + abs2(img[i, j] - img[i, j+1]))
...
function LR_TV(measured, psf, λ, N)
	...
	for i in 1:N
		ratio = measured ./ conv(img, psf)
		gradTV = gradient(TV, img)[1]
		img = conv(ratio, psf) .* img ./ (1 .- λ .* gradTV)
	end
	# LR with TV
	return img
end
...
function gauss(I, σ::T, x_offset::T, y_offset::T) where T
	...
	meshx = repeat((1:256)', 256, 1)
	gauss_x = exp.(-((meshx .- x_offset) .^ T(2)) ./ T(2 * σ^2))

	meshy = repeat((1:256), 1, 256)
	gauss_y = exp.(-((meshy .- y_offset) .^ T(2)) ./ T(2 * σ^2))

	Gauss = I .* gauss_x .* gauss_y
	return Gauss
end
...
# ╔═╡ 1b55f92a-f41c-4d51-8e6b-d31175de06ba
G = gauss(1.0, 1.0, 10.0, 10.0);

# ╔═╡ 858e31a0-edc7-47e4-92bb-881c7d90562c
G[11, 11]
...
function loss(measurement, pred)
	...
	return sum(abs2, (measurement - pred))
end
...
begin
	...
	bin_factor = 7 # TODO an integer number which produces a good guess
...
manifest_format = "2.0"
project_hash = "86917d73af7eb0b4d1f32b417427f53ffb9ea12b"
...
[[deps.DeconvOptim]]
deps = ["ChainRulesCore", "FFTW", "FillArrays", "Interpolations", "LineSearches", "LinearAlgebra", "Noise", "Optim", "PrecompileTools", "Requires", "Statistics", "StatsBase", "Tullio", "Zygote"]
git-tree-sha1 = "4257575512979d48424448b1cb44e29d6fd5fd4a"
...
[[deps.MicroscopyTools]]
deps = ["ColorTypes", "FFTW", "IndexFunArrays", "NDTools", "Statistics", "Tullio"]
git-tree-sha1 = "9d92e3de449d984a645997feb9572eeb8edaf145"
