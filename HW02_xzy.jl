...
```
"""
function to_equal_intensity_tuple(x::T) where T
	...
	red_map(x) = (T(1) .+ cos(T(2π) .* x)) / T(3)
	
	green_map(x) = (T(1) .+ cos(T(2π) .* x + T(2π/3))) / T(3)
	
	blue_map(x) = (T(1) .+ cos(T(2π) .* x + T(4π/3))) / T(3)
		
	return (red_map(x), green_map(x), blue_map(x)) # TODO, return the correct tuple!
end

# ╔═╡ 2ad7fbdd-b761-4993-87eb-56318e399560
typeof(Float32.(2π/3))
...
```
"""
function to_negative_positive_tuple(x::T) where T
...
function create_negative_positive_colormap()
	values = range(0, 1, length=128)
	return map(x -> RGB(x...), to_negative_positive_tuple.(values))
end

# ╔═╡ 32f90cbe-4ad5-4033-ba81-3cebe01f165d
begin
	my_negative_positive_colormap = create_negative_positive_colormap()
...
function simulate_pixel(n_photons; read_σ=5, offset=0.5, gain=0.1)
	poisson_noise = pois_rand.(n_photons) 
	gaussian_noise = read_σ .* randn(size(n_photons))
	noise = poisson_noise .+ gaussian_noise
	gained = noise .* gain
	signal = gained .+ offset
	return signal
end
...
# ╔═╡ 91782267-7395-4f70-9285-33c93242c009
[123 - 12.3, 123 + 12.3]

# ╔═╡ cfa08113-fb50-47c2-82d7-8235dcf629eb
var(simulate_pixel.(ones((1000,)) * 123, read_σ=0, offset=0.0123, gain=1))

# ╔═╡ 35d14b0e-22ff-4783-8f29-e2342856bd32
[123 - 12.3, 123 + 12.3]

# ╔═╡ 090361bb-22d6-4747-81d5-f73f50a203e9
std(simulate_pixel.(zeros((1000,)) * 123, read_σ=12.3, offset=0, gain=1))

# ╔═╡ 26db52ec-1ca1-4e38-ad1b-a56da20d9c8e
[12.3 - 1.23, 12.3 + 1.23]

# ╔═╡ ebc9cefd-ac14-4dc5-ad42-8f871960aad9
std(simulate_pixel.(zeros((1000,)) * 123, read_σ=12.3, offset=0, gain=42))

# ╔═╡ 6a70e09f-2f80-4f8e-8c1a-7d88a8a0f2ac
[516.6 - 51.66, 516.6+51.66]
...
# ╔═╡ 9ad8f91d-621d-4a9a-a1fa-66368bc79f77


# ╔═╡ b01359d9-c1b2-455f-897c-8ab6bed4f20f
# check variance poisson
PlutoTest.@test 123 ≈ var((x -> simulate_pixel(x; read_σ=0, offset=0.0123, gain=1)).(ones((100,)) * 123)) rtol=0.1

# ╔═╡ 73b6cea2-13d8-4f2d-8111-d4f858272ab1


# ╔═╡ a598763f-63f9-48c9-9e5b-ba6256775221
# check readnoise
PlutoTest.@test 12.3 ≈ std((x -> simulate_pixel(x; read_σ=0, offset=0.0123, gain=1)).(ones((100,)) * 123)) rtol=0.1
...
@. model(x, p) = p[1] .* x .+ p[2] #p[1] is gain, p[2] offset

# ╔═╡ ca78fb0c-3cd6-461e-a4c7-5bbd61589001
p0 = [0.5, 0.5] # TODO (maybe you need to convert it to Float32, maybe not), initial guess
...
manifest_format = "2.0"
project_hash = "1d75d37f6d760d0851f3f0ee7ab6f35637ef3c94"
