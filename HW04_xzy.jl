...
# ╔═╡ c16d3c64-0a6e-4ddd-9c5d-d8c73c7e80cc
my_conv([1.0,2,3,4], [0,1.0,0,0])
...
function calc_psf(arr_size, radius)
	arr = zeros(arr_size)
	center1 = arr_size[1] .÷ 2 .+ 1
	center2 = arr_size[2] .÷ 2 .+ 1
	arr[center1, center2] = 1.0
	
	ft_arr = ft(arr) .* circ(arr_size, radius)
	h = abs2.(ift(ft_arr))
	# return h
	return h ./ sum(h)
end
...
function wiener_filter(img, h, ϵ)
	# TODO
	filtered_img = ifft(fft(img) .* conj(fft(h)) ./ (abs2.(fft(h)) .+ ϵ))
	return real.(filtered_img)
end
...
# ╔═╡ 64c9b533-0abb-49d3-a263-5ad10f7f5603
Gray.(real.(ifft(fft(img))))
...
function loss(imgs::Vector{<:AbstractArray{T, N}}, μ::AbstractArray{T, N}) where {T, N}
	...
	for img in imgs
		for i in eachindex(img)
			loss += (img[i] - strange_sensor_f(μ[i]))^2
		end
	end
	return loss
end
...
function grad_strange_sensor_f(value::T) where T
	# TODO
	h = 1e-7
	grad = (strange_sensor_f.(value .+ h) .- strange_sensor_f.(value .- h)) ./ (2 * h)
	return grad
end

# ╔═╡ 01de98fe-5537-4745-acf8-99daa0b8aa6f
grad_strange_sensor_f(42)
...
# ╔═╡ 69ae3e93-65b7-4f07-9817-eb04da9ec1e2
function gradient(imgs::Vector{<:AbstractArray{T, N}}, μ::AbstractArray{T, N}) where {T, N}
	grad = zeros(size(μ))

	for img in imgs
		for i in eachindex(μ)
			grad[i] += -T(2) * (img[i] - strange_sensor_f(μ[i])) * grad_strange_sensor_f(μ[i])
		end
	end
	return grad
end
...
# ╔═╡ 9785bdcc-784f-4d21-813b-76ccdf99b532
md"""
...
# ╔═╡ f3e80045-2963-46a0-9305-0b8eec9f403c
md"""
Number of Iterations: 100
...
manifest_format = "2.0"
project_hash = "ef31e8890db58e7d5978a2acc91d0b678ba36fd9"
